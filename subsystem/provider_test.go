...
var _ = Describe("Provider subsystem tests", func() {
	...
	It("install cluster with oVirt hosts", func() {
		...
		generateEssentialHostStepsWithInventory(ctx, h1, "h2", inventoryInfo)
		generateEssentialHostStepsWithInventory(ctx, h1, "h3", inventoryInfo)
		apiVip := "1.2.3.8"
		ingressVip := "1.2.3.9"
	...
	It("install cluster with 2 ovirt hosts and one generic", func() {
		...
		generateEssentialHostStepsWithInventory(ctx, h1, "h2", inventoryOvirtInfo)
		generateEssentialHostStepsWithInventory(ctx, h1, "h3", inventoryBMInfo)
		apiVip := "1.2.3.8"
		ingressVip := "1.2.3.9"
		_, err := userBMClient.Installer.UpdateCluster(ctx, &installer.UpdateClusterParams{
			ClusterUpdateParams: &models.ClusterUpdateParams{
